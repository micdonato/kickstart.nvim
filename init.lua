...
do
  ...
  -- Load personal options early so plugin setup can use values like
...
do
  ...
  local function shorten_path(path)
    ...
    if home and path:find(home, 1, true) then path = path:gsub(vim.pesc(home), '~') end
    ...
    if #parts > 2 then return '...' .. table.concat(parts, '/', #parts - 2) end
...
do
  ...
  -- Route `vim.notify` through fidget so notifications render in its UI
  -- instead of the plain message area.
  vim.notify = require('fidget').notify

  -- Browse past notifications in a Telescope picker. Fidget's own
  -- `:Fidget history` only echoes them into the message area.
  pcall(require('telescope').load_extension, 'fidget')
  vim.keymap.set('n', '<leader>n', '<cmd>Telescope fidget<cr>', { desc = 'Search [N]otifications' })
  ...
  vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(event)
      ...
      if not vim.g.vscode and client and client:supports_method('textDocument/documentHighlight', event.buf) then
